...
`findings/` + `retros/`, the front-matter shape, the commit-trailer convention, the
`vcs_shape` branch/PR shapes and the one-ticket-one-commit bundle) are harness-specific — a new adapter should point at the same files and conventions the
