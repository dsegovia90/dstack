...
| 1 | Command arguments — `/dstack-ticket <project \| ticket-id \| LINEAR-ID>`, same for `/dstack-yolo`, `/dstack-retro`, `/dstack` | `[~]` | `feat/command-args` | #7 | An argument picks the project or *proposes* a ticket. The findings gate and the dependency check never move — a proposed ticket still goes through both. |
...
| 3 | `/dstack-feedback` — log a note about dstack itself while working | `[ ]` | `feat/feedback` | — | One file per note in `doc/dstack/feedback/`, `status: new`, same shape as a finding. A gate (same step as the findings scan) clears the list: **file** as a GitHub issue on the dstack repo via `gh`, or **drop** with a reason. Agents are told to log friction the moment they hit it. |
